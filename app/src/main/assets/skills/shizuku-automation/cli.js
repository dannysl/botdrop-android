...
function isSharedDirectoryCandidate(candidate = '') {
    ...
    return (normalized === '/data/local/tmp'
...
function sanitizeForLog(value, depth = 0, seen = new Set()) {
    ...
    if (Array.isArray(value)) {
        const limitedItems = value.slice(0, LOG_MAX_ARRAY_LENGTH).map((item) => sanitizeForLog(item, depth + 1, seen));
...
function getPostLaunchWaitOptions(flags) {
    const timeoutMs = parsePositiveInt(flags && flags['post-launch-timeout-ms'], LAUNCH_STABILITY_TIMEOUT_MS, 1);
    const stableCycles = parsePositiveInt(flags && flags['post-launch-stable-cycles'], LAUNCH_STABILITY_STABLE_CYCLES, 1);
    const pollMs = parsePositiveInt(flags && flags['post-launch-settle-ms'], LAUNCH_STABILITY_POLL_MS, 50);
...
async function waitForForegroundPackage(actions, packageName, options) {
    ...
    const timeoutMs = parsePositiveInt(options && options.timeoutMs, LAUNCH_STABILITY_TIMEOUT_MS, 1);
    const stableCycles = parsePositiveInt(options && options.stableCycles, LAUNCH_STABILITY_STABLE_CYCLES, 1);
    const pollMs = parsePositiveInt(options && options.pollMs, LAUNCH_STABILITY_POLL_MS, 50);
...
function appendCallLog(payload) {
    ...
    try {
        ...
        process.stderr.write(`[cli-log] write failed: ${error && error.message ? error.message : String(error)}\n`);
    }
}
...
async function main() {
    ...
    try {
        switch (command) {
            ...
            case 'screenshot': {
                ...
                if (hasTermuxFlag(args.flags, 'base64', 'as-base64')) {
                    ...
                    const hasImageTransform = args.flags.width !== undefined ||
                    ...
                    if (!localResolved.ok) {
                        fail('TERMUX_FILE_UNREADABLE', 'Failed to resolve screenshot path in Termux', {
            ...
            case 'launch': {
                ...
                if (!pkg)
                    fail('INVALID_ARGS', 'Usage: launch <package> [activity]');
            ...
            case 'kill': {
                const pkg = args.positional[0];
                if (!pkg)
                    fail('INVALID_ARGS', 'Usage: kill <package>');
            ...
            case 'tap': {
                ...
                if (isNaN(x) || isNaN(y))
                    fail('INVALID_ARGS', 'Usage: tap <x> <y>');
            ...
            case 'tap-element': {
                ...
                if (!raw)
                    fail('INVALID_ARGS', 'Usage: tap-element \'{"text":"OK"}\'');
            ...
            case 'press': {
                ...
                if (!key)
                    fail('INVALID_ARGS', 'Usage: press <key> (home/back/enter/recent/paste/...)');
            ...
            case 'wait-for': {
                const raw = args.positional[0];
                if (!raw)
                    fail('INVALID_ARGS', "Usage: wait-for '{\"text\":\"OK\"}' [--timeout ms]");
                const selector = parseSelector(raw);
            ...
            case 'ocr': {
                ...
                if (!imagePath)
                    fail('INVALID_ARGS', 'Usage: ocr <imagePath> [--timeout ms]');
            ...
            case 'latest-tweet': {
                ...
                const minTextLength = Number.isFinite(minTextLengthRaw) && minTextLengthRaw > 0 ? minTextLengthRaw : 20;
            ...
            case 'read-file': {
                ...
                if (!filePath)
                    fail('INVALID_ARGS', 'Usage: read-file <path> [--max-bytes N] [--base64]');
            ...
            case 'image-meta': {
                const imagePath = args.positional[0];
                if (!imagePath)
                    fail('INVALID_ARGS', 'Usage: image-meta <imagePath>');
                const localResolved = resolveReadableLocalPath(imagePath);
                if (!localResolved.ok) {
            ...
            case 'image-to-base64': {
                const imagePath = args.positional[0];
                if (!imagePath)
                    fail('INVALID_ARGS', 'Usage: image-to-base64 <imagePath> [--format png|jpeg|webp] [--width N] [--height N]');
                const localResolved = resolveReadableLocalPath(imagePath);
                if (!localResolved.ok) {
                    fail('LOCAL_FILE_NOT_FOUND', 'No readable image path in Termux', {
                        requestedPath: imagePath,
            ...
            case 'exec': {
                ...
                if (!cmd)
                    fail('INVALID_ARGS', 'Usage: exec <shell command>');
        ...
        if (err && typeof err === 'object') {
            ...
            if (err.originalCode)
                failExtra.originalCode = err.originalCode;
            if (err.originalMessage)
                failExtra.originalMessage = err.originalMessage;
    ...
    return new Promise((resolve) => {
        execInShell(shellCommand, {
            ...
            if (error) {
                ...
                const exitCode = typeof error.code === 'number' ? error.code : isTimeout ? 124 : 1;
                const stdoutText = String(stdout || '');
                const combinedStderr = `${String(stderr || '')}${stderr && error.stderr ? '\n' : ''}${error.stderr || ''}`.trim();
                const isExpectedPipeClose = exitCode === 141 &&
                    !combinedStderr &&
                    /\\|/.test(cmd) &&
                    !!stdoutText;
...
async function readImageText(imagePath, timeoutMs = 30000, maxCharsRaw = null) {
    ...
    if (!whichTesseract.ok || !String(whichTesseract.stdout || '').trim()) {
        return {
            ...
            message: 'No OCR tool found in Termux shell. Install tesseract (pkg install tesseract) to use ocr.',
        };
    }
    ...
    const maxChars = parseInt(maxCharsRaw, 10);
    const clippedText = Number.isFinite(maxChars) && maxChars > 0 && text.length > maxChars ? text.slice(0, maxChars) : text;
    return {
        ok: true,
        mode: 'termux',
        path: pathText,
...
function extractLatestTweetFromUiDump(elements, options = {}) {
    ...
    const rowElements = rows.filter((el) => {
        if (!el || !el.resourceId)
            return false;
        if (el.resourceId !== 'com.twitter.android:id/row')
            return false;
        if (packageName && el.packageName && el.packageName !== packageName)
            return false;
        if (!el.bounds)
            return false;
        return true;
    ...
    const candidates = rows.filter((el) => {
        if (!el || !el.text)
            return false;
        if (!isInsideBounds(el.bounds, latestRow.bounds))
            return false;
        if (!isLikelyTextForFeed(el.text, minTextLength))
            return false;
        return true;
    });
    candidates.sort((a, b) => {
...
function isLikelyTextForFeed(text, minLength = 20) {
    ...
    if (!normalized || normalized.length < minLength)
        return false;
    if (/^\d{1,4}\s*(分钟前|小时前|天前|月前|周前|年)/.test(normalized))
        return false;
    if (/^(0|[1-9]\d*)\s*个?\s*(转帖|喜欢|查看次数|回复)?$/.test(normalized))
        return false;
    if (normalized === '广告' || normalized === '已发布')
        return false;
    if (normalized.includes('认证查看次数'))
        return false;
    if (normalized.includes('显示更多'))
        return false;
    if (normalized.includes('认证企业'))
        return false;
    return true;
}
function isInsideBounds(child, parent) {
    if (!child || !parent)
        return false;
    return (Number.isFinite(child.left) && Number.isFinite(child.top) &&
