...
function matchesSelector(el, selector) {
    ...
    if (selector.text !== undefined && el.text !== selector.text)
        return false;
    if (selector.textContains !== undefined && !el.text.includes(selector.textContains))
        return false;
    if (selector.resourceId !== undefined && el.resourceId !== selector.resourceId)
        return false;
    if (selector.className !== undefined && el.className !== selector.className)
        return false;
    if (selector.description !== undefined && el.description !== selector.description)
        return false;
    if (selector.descriptionContains !== undefined && !el.description.includes(selector.descriptionContains))
        return false;
    if (selector.clickable !== undefined && el.clickable !== selector.clickable)
        return false;
    if (selector.enabled !== undefined && el.enabled !== selector.enabled)
        return false;
    if (selector.packageName !== undefined && el.packageName !== selector.packageName)
        return false;
    return true;
}
class UIEngine {
    ...
    async dump() {
        ...
        if (!dumpRes.ok) {
            throw Object.assign(new Error(`UI dump command failed: ${dumpRes.error || dumpRes.message || 'unknown error'}`), {
                code: 'DUMP_FAILED',
        ...
        if (!(res.ok && res.stdout && res.stdout.includes('<hierarchy'))) {
            throw Object.assign(new Error(`UI dump failed: ${res.stderr || res.message || 'output missing or invalid'}`), {
                code: 'DUMP_FAILED',
    ...
    async waitFor(selector, timeoutMs = DEFAULT_WAIT_TIMEOUT_MS) {
        ...
        throw Object.assign(new Error('Element not found within ' + timeoutMs + 'ms: ' + JSON.stringify(selector)), { code: 'ELEMENT_NOT_FOUND' });
    }
    /**
    ...
    async tap(selector) {
        const el = await this.findOne(selector);
        if (!el) {
            throw Object.assign(new Error('Element not found: ' + JSON.stringify(selector)), { code: 'ELEMENT_NOT_FOUND' });
        }
        if (!el.center) {
            throw Object.assign(new Error('Element has no bounds: ' + JSON.stringify(selector)), { code: 'ELEMENT_NO_BOUNDS' });
        }
